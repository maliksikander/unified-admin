...
const { emailAutoResponsesService } = require('../services');
const logger = require('../config/logger');
const { v4: uuidv4 } = require("uuid");

const getAutoResponses = catchAsync(async (req, res) => {
    ...
    try {
        logger.info(`Get Auto Responses`, { className: "emailAutoResponses.controller", methodName: "getAutoResponses", CID: coId });
        ...
        if (channelIdentifier) {
            ...
            result = await emailAutoResponsesService.getAutoResponsesByChannelIdentifier(channelIdentifier, coId);
            if (result.length === 0) {

                return res.status(404).json({
                    status: 404,
                    message: `Email Auto Responses Not Found against channel Identifer: ${channelIdentifier}`,
                    response: []
                });
            }
            ...
            result = await emailAutoResponsesService.getAutoResponses(coId);
        }
        ...
        if (result.length === 0) {
            ...
            res.status(404).json({
                status: 404,
                message: "No auto Responses found.",
                response: []
            });
        } else {
        ...
        logger.error(`[ERROR] on get auto responses api: %o` + error, { className: "emailAutoResponses.controller", methodName: "getAutoResponses", CID: coId });
    }
});
